...
def compute_like(ts_sim_file,**params):
    "dt_step incremental with sim"
    head,tail = os.path.split(ts_sim_file)
    sim = (tail.split('_sim')[1]).split('.')[0]
    dt_step = 0.01 + int(sim) * 0.001    # Increase dt_step with simulation for Ne growing 
    if debug: print('dt_step', dt_step)
    
    "load ts file as ts"
    ts = tskit.load(ts_sim_file)
...
def like_profile(ts_sim_file,params,true_val):
    
    profile_vals = list(np.arange(50,400,10)) # for true val = 200
    like_vals = []
    for val in profile_vals:
        params['Ne'] = val
        like_vals.append(compute_like(ts_sim_file,**params))
...
def like_profile_rho(ts_sim_file,params,true_val):
...
def like_profile_M(ts_sim_file,params,M):
